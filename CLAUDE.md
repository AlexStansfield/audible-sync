...
Still to come in Milestone 3: the extras (log endpoint, webhook on run finished, stats). The plan for those is in `todo.md`.
...
**Milestone 3 - in progress:** settings table, the service runtime (scheduler, run state, cancel, status/sync/runs/settings endpoints, bearer token, Docker runs the service), accounts (one per marketplace login, credentials in the database, per-account pipeline and endpoints), the browser login flow and book management (listing, monitor, delete, redownload, retry, refresh, cover) are done (2026-09-12). Next: the extras (log endpoint, webhook, stats). **Step 0 is complete** (2026-09-09), including the smaller items folded in with it.
...
**Codebase Version:** Milestone 2 complete; Milestone 3 Step 0, the settings table, the service runtime, accounts, the login flow and book management complete (scheduler, run state, cancellation, bearer-token API for status/sync/runs/settings/accounts/login/books; library keyed per account; the Docker image runs the service)
**Primary Branch:** `dev`
